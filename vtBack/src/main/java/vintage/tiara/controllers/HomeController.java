...
private static final Logger LOGGER = Logger.getLogger(LogInController.class.getName());
	...
	@RequestMapping(value="/loadAllProducts", method = RequestMethod.GET)
	@ResponseBody public ResponseEntity<List<Product>> getAllProducts(){
		List<Product> dataList = new ArrayList<Product>();
		HttpStatus status = HttpStatus.OK;
		
		try {
			dataList = (List<Product>) dataS.getAllProducts();
		}catch (Exception e) {
			LOGGER.log(Level.FINE, "Something went wrong", e);
			status = HttpStatus.NOT_FOUND;
			return new ResponseEntity<List<Product>>(dataList, status);
